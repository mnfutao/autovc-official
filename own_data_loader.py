...
class Utterances(data.Dataset):
    ...
    def __init__(self, root_dir, len_crop):
        ...
        self.num_tokens = sum([ len(x) - 2 for x in self.train_dataset])
        self.datasets = []

        for c_speaker_data in  self.train_dataset:
            self.datasets.extend(c_speaker_data)

        print()
        print(f'c_speaker_data-->num:{len(self.datasets)}')
        print(f'num_tokens-->num:{len(self.datasets)}')
        ...
        for k, sbmt in enumerate(submeta):    
            ...
            for j, tmp in enumerate(sbmt):
                if j < 2:  # fill in speaker id and embedding
                    ...
                    uttrs[j] = [np.load(os.path.join(self.root_dir, tmp)), uttrs[1], uttrs[0]]
            dataset[idx_offset+k] = uttrs[2:]
    ...
    def __getitem__(self, index):
        ...
        speaker_id = list_uttrs[2]
        data_index = [index]
        return uttr, emb_org, speaker_id, data_index
